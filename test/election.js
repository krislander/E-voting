...
contract("Election", function(accounts) {
  ...
  it("it initializes candidates with the correct values", function() {
    return Election.deployed().then(function(instance) {
      ...
      assert.equal(candidate[2], "Republicans", "contains the correct party name");
      assert.equal(candidate[3].toNumber(), 0, "contains the correct votes count");
      ...
      assert.equal(candidate[2], "Democrats", "contains the correct party name");
      assert.equal(candidate[3].toNumber(), 0, "contains the correct votes count");
    });
  });
  ...
  it("allows a voter to vote", function() {
    return Election.deployed().then(function(instance) {
      ...
      assert.equal(receipt.logs[0].event, "VoteInElection", "the event type is correct");
  ...
  it("throws an exception for invalid candidates", function() {
    return Election.deployed().then(function(instance) {
      ...
      var voteCount = candidate1[3].toNumber();
      assert.equal(voteCount, 1, "candidate 1 did not receive any votes");
      return electionInstance.candidates(2);
    }).then(function(candidate2) {
      var voteCount = candidate2[3].toNumber();
